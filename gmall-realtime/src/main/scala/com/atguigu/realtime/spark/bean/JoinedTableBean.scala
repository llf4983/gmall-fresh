package com.atguigu.realtime.spark.bean

import java.text.SimpleDateFormat

case class JoinedTableBean(
  ...
  def this(orderInfo:OrderBean,orderDetail:OrderDetail){
    this
    combineOrder(orderInfo)
    combineDetail(orderDetail)
  }

  def combineOrder(orderInfo:OrderBean): Unit ={
    if(orderInfo!=null){
  ...
  def combineDetail(orderDetail:OrderDetail): Unit ={
    if(orderDetail!=null){
  ...
  def combineUser(userInfo: UserInfo): Unit ={
    if(userInfo!=null){
